...
void blackout(struct SpaceGlobals* mySpaceGlobals)
{
	fillScreen(0,0,0,0, mySpaceGlobals->screen);
}
...
//	Vec2D right = mySpaceGlobals->rstick;
	...
	// get the differences
    float xdif = 0; // left.x + right.x;
    float ydif = 0; // left.y + right.y;
	
	// Handle D-pad movements as well
	// max out speed at 1 or -1 in both directions
	xdif = (xdif >  1 || mySpaceGlobals->button & KEY_RIGHT)?  1 : xdif;
	xdif = (xdif < -1 || mySpaceGlobals->button &  KEY_LEFT)? -1 : xdif;
	ydif = (ydif >  1 || mySpaceGlobals->button &	KEY_UP)?  1 : ydif;
	ydif = (ydif < -1 || mySpaceGlobals->button &  KEY_DOWN)? -1 : ydif;
...
{
	if (mySpaceGlobals->button & KEY_START)
	{
...
void renderEnemies(struct SpaceGlobals *mySpaceGlobals)
{
	...
	for (x=0; x<20; x++)
	{
		if (mySpaceGlobals->bullets[x].active == 1)
		{
			...
			for (z=0; z<4; z++)
				for (za=0; za<2; za++)
					drawPixel(mySpaceGlobals->bullets[x].x + z, mySpaceGlobals->bullets[x].y + za, 255, 0, 0,  mySpaceGlobals->screen);
		}
	}
	...
	for (x=0; x<100; x++) // up to 100 enemies at once
	{
		if (mySpaceGlobals->enemies[x].position.active >= 1)
		{
			drawBitmap(mySpaceGlobals->enemies[x].position.x, mySpaceGlobals->enemies[x].position.y, 23, 23, mySpaceGlobals->enemies[x].rotated_sprite, enemy_palette, mySpaceGlobals->screen);
		}
	}
}
...
{
	fillRect(0, 0, xMaxBoundry, 20, 0, 0, 0, mySpaceGlobals->screen);
	...
	drawString(0, -1, score, mySpaceGlobals->screen);
	
	char level[255];
	snprintf(level, 255, "Lv %d", mySpaceGlobals->level+1);
	drawString(30, -1, level, mySpaceGlobals->screen);
	

	char lives[255];
	snprintf(lives, 255, "Lives: %d", mySpaceGlobals->lives);
	drawString(55, -1, lives, mySpaceGlobals->screen);
	...
	if (mySpaceGlobals->displayHowToPlay)
	{
		...
		drawString(20, 17, nag, mySpaceGlobals->screen);
...
void renderShip(struct SpaceGlobals *mySpaceGlobals) 
{
	...
	drawBitmap(posx, posy, 36, 36, mySpaceGlobals->rotated_ship, mySpaceGlobals->curPalette, mySpaceGlobals->screen);
...
void renderStars(struct SpaceGlobals *mySpaceGlobals)
{
	...
	drawPixels(mySpaceGlobals->stars, mySpaceGlobals->screen);
}
...
void displayTitle(struct SpaceGlobals * mySpaceGlobals)
{
	if (mySpaceGlobals->invalid == 1)
	{
		...
		drawString(37, 9, credits, mySpaceGlobals->screen);
		
		drawString(25, 12, play, mySpaceGlobals->screen);
		
		drawString(26, 13, password, mySpaceGlobals->screen);
...
//	fillRect(240, 155, 16, 30, 0, 0, 0);
	...
	char cursor[255];
	snprintf(cursor, 255, ">>            <<");
	drawString(22, 12 + mySpaceGlobals->menuChoice, cursor, mySpaceGlobals->screen);
...
void doMenuAction(struct SpaceGlobals *mySpaceGlobals)
{
	...
	if (!(mySpaceGlobals->button & KEY_A))
	{
		mySpaceGlobals->allowInput = 1;
	}
	
	if (mySpaceGlobals->button & KEY_A && mySpaceGlobals->allowInput)
	{
...
//	float stickY = mySpaceGlobals->lstick.y + mySpaceGlobals->rstick.y;
	...
	if (mySpaceGlobals->button & KEY_DOWN || stickY < -0.3)
	{
		mySpaceGlobals->menuChoice = 1;
		mySpaceGlobals->invalid = 1;
	}
	
	if (mySpaceGlobals->button & KEY_UP || stickY > 0.3)
	{
		mySpaceGlobals->menuChoice = 0;
		mySpaceGlobals->invalid = 1;
	}
...
void displayPause(struct SpaceGlobals * mySpaceGlobals)
{
	if (mySpaceGlobals->invalid == 1)
	{
		...
		drawString(27, 12, resume, mySpaceGlobals->screen);
		
		drawString(28, 13, quit, mySpaceGlobals->screen);
...
void doPasswordMenuAction(struct SpaceGlobals * mySpaceGlobals)
{
	...
	if (!(mySpaceGlobals->button & KEY_A	 ||
		  mySpaceGlobals->button & KEY_UP	||
		  mySpaceGlobals->button & KEY_DOWN  ||
		  mySpaceGlobals->button & KEY_LEFT  ||
		  mySpaceGlobals->button & KEY_RIGHT   ))
	{
		mySpaceGlobals->allowInput = 1;
	}
	...
	{
		if (mySpaceGlobals->button & KEY_B)
		{
        ...
        float stickY = 0; //mySpaceGlobals->lstick.y + mySpaceGlobals->rstick.y;
        float stickX = 0; // mySpaceGlobals->lstick.x + mySpaceGlobals->rstick.x;
		int down  = (mySpaceGlobals->button & KEY_DOWN  || stickY < -0.3);
		int up	= (mySpaceGlobals->button & KEY_UP	|| stickY >  0.3);
		int left  = (mySpaceGlobals->button & KEY_LEFT  || stickX < -0.3);
		int right = (mySpaceGlobals->button & KEY_RIGHT || stickX >  0.3);
...
//		drawPasswordMenuCursor(mySpaceGlobals);
		...
		drawString(22, 8, password, mySpaceGlobals->screen);
		
		
		drawString(32 + mySpaceGlobals->menuChoice, 7, up_cur, mySpaceGlobals->screen);
		
		drawString(32, 8, cur_pw, mySpaceGlobals->screen);
		
		drawString(32 + mySpaceGlobals->menuChoice, 9, down_cur, mySpaceGlobals->screen);
...
void displayGameOver(struct SpaceGlobals *mySpaceGlobals)
{
	if (mySpaceGlobals->invalid == 1)
	{
		...
		drawString(25, 5, gameover, mySpaceGlobals->screen);
		...
		if (mySpaceGlobals->dontKeepTrackOfScore != 1)
		{
			...
			drawString(23, 7, finalscore, mySpaceGlobals->screen);
			
			drawString(21, 8, pass, mySpaceGlobals->screen);
		...
		drawString(25, 12, resume, mySpaceGlobals->screen);
